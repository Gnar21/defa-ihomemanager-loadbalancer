...
class AsyncModbusEndpointClient:
    ...
    PyModbus API differs across versions: some use slave=, some use unit=.
    This wrapper supports both by trying slave= first and falling back to unit=. [1](https://stackoverflow.com/questions/74921683/pymodbus-read-input-registers-error-modbusioexception-object-has-no-attribute)[2](https://www.pymodbus.org/docs/reading-registers)
    """
    ...
    async def read_input_registers(self, address: int, count: int):
        async with self._lock:
            ...
            # Try modern keyword first (slave=), fallback to older (unit=)
            try:
                ...
                return await self._client.read_input_registers(
                    address=address, count=count, unit=self._ep.unit
                )
    ...
    async def write_registers(self, address: int, values: list[int]):
        async with self._lock:
            ...
            try:
                ...
                return await self._client.write_registers(
                    address=address, values=values, unit=self._ep.unit
                )
