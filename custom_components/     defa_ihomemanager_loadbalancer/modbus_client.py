...
class AsyncModbusEndpointClient:
    ...
    async def read_input_registers(self, address: int, count: int):
        async with self._lock:
            ...
            return await self._client.read_input_registers(address=address, count=count, slave=self._ep.unit)

    async def write_registers(self, address: int, values: list[int]):
        async with self._lock:
            await self.async_connect()
            assert self._client is not None
            return await self._client.write_registers(address=address, values=values, slave=self._ep.unit)
...
def u32_hybrid_low_word_first(regs: list[int], idx: int = 0) -> int:
    # iHM U32: low-word first [8](https://www.teknikihemmet.se/2025/04/installera-och-anvanda-hacs-integration-i-home-assistant/)
    low = regs[idx] & 0xFFFF
